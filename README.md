...
### Дополнительные возможности и Автоуведомления

Бот поддерживает автоматические напоминания об оплате и секретную **Админ-панель**.
Для их настройки добавьте следующие переменные в файл `.env`:

```env
# ID администраторов через запятую (например, 12345678,87654321). 
# Только эти пользователи смогут вызвать команду /admin
ADMIN_TELEGRAM_IDS=123456789

# Настройки уведомлений об оплате (опционально)
PAYMENT_NOTIFICATION_DAY=25
PAYMENT_NOTIFICATION_TIME=10:00
PAYMENT_NOTIFICATION_MESSAGE=Пожалуйста, оплатите подписку на VPN, иначе доступ будет приостановлен.
```

### 👑 Админ-панель (Команда `/admin`)
Если ваш `telegramId` указан в `ADMIN_TELEGRAM_IDS`, вы можете отправить боту команду `/admin`.
Вам откроется секретное меню:
1. **Общий список пользователей:** Просмотр списка всех клиентов (кто оплатил, а кто просрочил). Под списком находятся кнопки для мгновенного продления подписки любому пользователю на +30 дней в один клик.
2. **Отправить рассылку всем:** Возможность отправить текстовое сообщение, кружок или картинку сразу всем вашим клиентам (идеально для новостей или напоминаний об оплате).
```
